...
st.set_page_config(page_title="OWTICS S21 MID META", page_icon="🔥", layout="wide")

# 가독성을 극대화한 오버워치 테마 CSS
st.markdown("""
<style>
...
st.markdown('<div class="ow-subtitle">시즌 21 미드 기준 실시간 영웅 통계 (총 50명)</div>', unsafe_allow_html=True)

# 유저가 제공한 완벽한 50명 전체 데이터
data = {
    '영웅': [
    ...
    '픽률(%)': [
        14.1, 11.4, 10.8, 10.1, 9.0, 8.7, 7.9, 6.8, 5.8, 4.3, 3.4, 3.1, 2.8, 1.8, 
        21.9, 18.2, 17.9, 15.0, 13.8, 13.4, 12.4, 11.8, 9.2, 8.4, 7.1, 6.3, 5.9, 5.6, 5.3, 4.4, 4.2, 4.2, 4.1, 4.0, 3.8, 3.3, 
        47.2, 7.7, 3.5, 12.6, 5.5, 10.8, 33.2, 3.9, 5.6, 8.9, 21.9, 15.7, 12.7, 10.8
    ],
    '승률(%)': [
        53.2, 53.8, 52.3, 45.2, 49.5, 48.1, 49.2, 52.0, 46.4, 40.9, 50.7, 46.3, 51.2, 47.0, 
        46.9, 49.7, 48.5, 45.8, 51.5, 51.0, 50.8, 51.1, 47.4, 50.2, 49.8, 48.2, 49.9, 52.3, 51.7, 47.2, 50.0, 53.7, 53.8, 51.8, 49.6, 49.2, 
        49.6, 45.3, 46.5, 54.4, 51.9, 49.9, 47.1, 43.6, 47.9, 47.0, 52.7, 47.8, 52.2, 52.3
    ]
}
...
# --- 3. 50명 전체 데이터 표 (가독성 끝판왕) ---
st.markdown("### 📋 FULL HERO LEADERBOARD")
st.markdown("컬럼 이름을 클릭하면 픽률이나 승률 순으로 정렬해서 볼 수 있습니다.")

# 스트림릿 내장 데이터프레임으로 깔끔하게 출력
st.dataframe(
    df[['영웅', '포지션', '세부역할', '픽률(%)', '승률(%)']],
    use_container_width=True,
    hide_index=True,
    column_config={
        "픽률(%)": st.column_config.ProgressColumn("픽률(%)", help="전체 픽률", format="%f%%", min_value=0, max_value=50),
        "승률(%)": st.column_config.NumberColumn("승률(%)", help="전체 승률", format="%f%%")
    }
)
