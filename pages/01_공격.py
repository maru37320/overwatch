...
st.markdown("<style>@import url('https://fonts.googleapis.com/css2?family=Teko:wght@600&display=swap'); h1, h2 { font-family: 'Teko', sans-serif; font-style: italic; color: #F4556C !important; }</style>", unsafe_allow_html=True)
...
with col1:
    ...
    fig_pick = px.bar(
        dmg_df.sort_values('픽률(%)', ascending=False),
        x='영웅', y='픽률(%)', text_auto='.1f',
        color='세부역할', color_discrete_sequence=px.colors.sequential.Reds_r
    )
...
with col2:
    ...
    fig_win = px.bar(
        dmg_df.sort_values('승률(%)', ascending=False),
