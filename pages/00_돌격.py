...
st.title("🛡️ TANK HERO METRICS (S21 MID)")

# 데이터 로드
if os.path.exists('owtics_s21_data.csv'):
    df = pd.read_csv('owtics_s21_data.csv')
else:
    st.error("메인 페이지(app.py)를 먼저 실행해서 데이터를 생성해 줘!")
    st.stop()
...
with col1:
    st.subheader("PICK RATE (%) - 시그마 1위")
    fig_pick = px.bar(
        tank_df.sort_values('픽률(%)', ascending=False),
        x='영웅', y='픽률(%)', text_auto='.1f',
        color='영웅', color_discrete_map={'시그마': '#f99e1a', '도미나': '#4EA8DE'}
    )
...
with col2:
    st.subheader("WIN RATE (%) - 도미나 53.8%")
    fig_win = px.bar(
